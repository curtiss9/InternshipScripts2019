...
ncfile = "C:\\Users\\sabrina.curtis\\Desktop\\Data\\models\\loveclim_mhdiff_2deg.nc"   # Load model
ncin <- nc_open(ncfile)

loveclim <- ncvar_get(ncin,"temp2")
...
ncfile = "C:\\Users\\sabrina.curtis\\Desktop\\Data\\models\\famous_mhdiff_2deg.nc"   # Load model
ncin <- nc_open(ncfile)

famous <- ncvar_get(ncin,"temp2")
...
ncfile = "C:\\Users\\sabrina.curtis\\Desktop\\Data\\models\\trace_mhdiff_2deg.nc"   # Load model
ncin <- nc_open(ncfile)

trace <- ncvar_get(ncin,"temp2")
...
traceEast <- trace[c(1:90),]
traceWest <- trace[c(91:180),]
trace <- rbind(traceWest, traceEast)

famousEast <- famous[c(1:90),]
famousWest <- famous[c(91:180),]
...
# remove those rows
avg_temp <- avg_temp[-c(87,88,89,90),]
lonNaRemoved <- lonL[-c(87,88,89,90)]
...
dev.copy(png, 'C:\\Users\\sabrina.curtis\\Desktop\\Maps\\models_averaged2x2.png')
dev.off()
...
# average into 2 degree grid?



##### RMSE For Average of 3 Models #####
...
weightModel <- weightModel[,-c(1,2,89,90)]
latNAremoved <- latL[-c(1,2,89,90)]
...
barplot(rmse, main = "RMSE of the Transient Model Average vs Reconstruction Data", 
	xlab = "Root Mean Square Error", ylab = "Latitude", col = "blue",
	horiz=TRUE, names.arg = latMeansModel$LatBin, las=1)
mtext("(Models Gridded into 2 Degree Bins)")

#dev.copy(png, 'C:\\Users\\sabrina.curtis\\Desktop\\Maps\\rmse_plot_2x2.png')
#dev.off()
...
# Remove NA rows and columns
weightModel <- weightModel[-c(179,180),-c(1,90)]
latNAremoved <- latT[-c(1,90)]
...
# Remove NA rows and columns
weightModel <- weightModel[-c(177,178,179,180),]
