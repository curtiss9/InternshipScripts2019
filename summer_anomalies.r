...
dataAnom <- subset(dataInput, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
dataAnom <- subset(dataAnom, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
data1 <- subset(dataNOAA, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
data2 <- subset(dataPANGAEA, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
dataCoreTops <- readWorkbook("C:\\Users\\sabrina.curtis\\Desktop\\Data\\present\\summer_coretops.xlsx", sheet=1)
...
green1 <- data.frame(Temp = dataGreen$JJA, Age = dataGreen$Age,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 NEEM",
				Lat = 77.45, Lon = -51.06)
green2 <- data.frame(Temp = dataGreen$JJA.1, Age = dataGreen$Age.1,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 NGRIP",
				Lat = 75.1, Lon = -42.32)
green3 <- data.frame(Temp = dataGreen$JJA.2, Age = dataGreen$Age.2,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 GISP2",
				Lat = 72.58, Lon = -38.48)
green4 <- data.frame(Temp = dataGreen$JJA.3, Age = dataGreen$Age.3,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 Dye-3",
				Lat = 65.2, Lon = -43.8)
green5 <- data.frame(Temp = dataGreen$JJA.4, Age = dataGreen$Age.4,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 ReCAP/Renland",
				Lat = 71.3, Lon = -26.72)
green6 <- data.frame(Temp = dataGreen$JJA.5, Age = dataGreen$Age.5,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 Agassiz",
				Lat = 80.82, Lon = -72.9)
green7 <- data.frame(Temp = dataGreen$JJA.6, Age = dataGreen$Age.6,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 Hans Tausen Iskappe 1995",
				Lat = 82.88, Lon = -36.47)
green8 <- data.frame(Temp = dataGreen$JJA.7, Age = dataGreen$Age.7,
				Proxy = "ice core", Seasonality = 'summer',
				Study = "Buizert 2018 Camp Century",
				Lat = 77.2, Lon = -61.1)
green9 <- data.frame(Temp = dataGreen$JJA.8, Age = dataGreen$Age.8,
				Proxy = "ice core", Seasonality = 'summer',
...
dataArctic <-  subset(dataArctic, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
arcticAnom <-  subset(arcticAnom, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
dataArctic2 <-  subset(dataArctic2, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
dataArctic3 <-  subset(dataArctic3, Seasonality == "summer" | Seasonality == "august" | 
				Seasonality == "warmest month" | Seasonality == "july" | 
				Seasonality == "warm month" | Seasonality == "warm season")
...
data <- rbind(dataAnom, dataAnom2, greenlandAnom, arcticAnom, arcticAnom2)
...
#write.csv(data, "C:\\Users\\sabrina.curtis\\Desktop\\Data\\exported\\temp_anomalies_summer.csv")
...
# Plot the map
plot(map, main = paste0("Summer Temperature Anomalies in 2 Degree Grid\n(n = ", numRows, ")" ))
# Bring in color palettes
cols <- rev(brewer.pal(10, "RdYlBu"))
cols <- colorRampPalette(cols)
...
#dev.copy(png, 'C:\\Users\\sabrina.curtis\\Desktop\\Maps\\proxy_summer2x2.png')
#dev.off()
...
# Plot the map
plot(map, main = paste0("Summer Temperature Anomalies\nin a 4 Degree Grid" ))
# Bring in color palettes
cols <- rev(brewer.pal(10, "RdYlBu"))
cols <- colorRampPalette(cols)
...
legend("bottom", col = cols(10), pch = 15, legend = levels(cut(data5DegGrid$Temp, breaks=10)),
		title = "Temperature Range (degrees C)", ncol = 5)

#dev.copy(png, 'C:\\Users\\sabrina.curtis\\Desktop\\Maps\\proxy_summer4x4.png')
#dev.off()
